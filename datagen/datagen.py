...
dirName = "newnew_tr_"+str(temp_res)+"_nn_"+str(num_nodes)+"_nr_"+str(num_rooms)+"_nc_"+str(int(covid_prob*100))
...
dataframe = dataframe.astype({'UserID': 'int32', 'EntranceGPSTime': 'int32','ExitGPSTime': 'int32','Duration':'float32'})  
...
df_meet =df_loc.merge(df_loc, left_on=['loc','Interval'], right_on=['loc','Interval']).rename(columns={'src_x':'src','src_y':'dst'})
#df_loc4 = df_loc3[df_loc3['Interval'] == df_loc3['Interval2']]
...
nn = nodes.groupby(["nid","label","prop1","prop2","prop3","cumsum"]).agg({'start':'min', 'end':'max'}).reset_index()
nodes = nn
