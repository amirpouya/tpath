...
temp_res = int(sys.argv[2]) #min
num_rooms =  10
covid_prob = .05


dirName = "tr_"+str(temp_res)+"_nn_"+str(num_nodes)+"_nr_"+str(num_rooms)+"_nc_"+str(int(covid_prob*100))
print("config ",dirName)


dataframe = pd.read_csv("bigfile.csv")
...
split_df = dataframe[dataframe["Interval"].map(len)>1]#.explode('Interval')
